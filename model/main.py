...
from lstm.lstmcrf import Bilstmcrf
from lstm.get_data import get_cyber_data,pregress
...
def evaluate(data, model, label_map, tag, args, train_logger,mode):
...
def train(model, train_dataloader, dev_dataloader, args, device, tb_writer, label_map, tag, train_logger):
    ...
    for epoch in tq:
        ...
        metric, metric_instance = evaluate(dev_dataloader, model, label_map, tag, args, train_logger,'dev')
        metric_instance['epoch'] = epoch
        metric['epoch'] = epoch

        print('epoch{} P{}, R{}, F1{}'.format(metric['precision-overall'],metric['recall-overall'],metric['f1-measure-overall']))
        logger.info('epoch{} P{}, R{}, F1{}'.format(metric['precision-overall'],metric['recall-overall'],metric['f1-measure-overall']))
        ...
        if metric_instance['micro-f1'] > bestscore_instance:
            ...
            print('实体级别的F1best model epoch is: %d' % epoch)
            train_logger.info('实体级别的F1best model epoch is: %d' % epoch)
...
def load_predict(model,data,model_save_dir,logger,label_map,tag,args):
    start_time = time.time()
    model.load_state_dict(torch.load(model_save_dir))
    
    metric,metric_instance = evaluate(test_dataloader, model, label_map, tag, args,logger,'test')
    end_time = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_save_dir', type=str, default='/opt/hyp/NER/NER-model/saved_models/bilstm/', help='Root dir for saving models.')
    ...
    parser.add_argument('--pred_embed_path', default='/opt/hyp/NER/embedding/Tencent_AILab_ChineseEmbedding.txt',type=str,
                            choices=['cc.zh.300.vec','sgns.baidubaike.bigram-char','Tencent_AILab_ChineseEmbedding.txt'], help='预训练词向量路径')
    parser.add_argument('--tensorboard_dir',default='/opt/hyp/NER/NER-model/saved_models/bilstm/runs/', type=str)
    ...
    train_data = json.load(open(args.data_path + '/train_data.json', encoding='utf-8'))
    test_data = json.load(open(args.data_path + '/test_data.json', encoding='utf-8'))
    dev_data = json.load(open(args.data_path + '/dev_data.json', encoding='utf-8'))
    train_logger.info('训练集大小为{}，验证集大小为{}，测试集大小为{}'.format(len(train_data),len(dev_data),len(test_data)))
    ...
    train_data_id, train_mask_id, train_label_id = pregress(train_data, word2idx, label2index, max_seq_lenth=args.max_seq_length)
    ...
    dev_data, dev_mask, dev_label = pregress(dev_data, word2idx, label2index, max_seq_lenth=args.max_seq_length)
    ...
    test_data, test_mask, test_label = pregress(test_data, word2idx, label2index, max_seq_lenth=args.max_seq_length)
    ...
    if args.do_train:
        dev_result, dev_result_instance = train(model, train_dataloader, dev_dataloader, args, device, tb_writer,  \
                                                index2label, tag, train_logger)
    ...
    if args.do_test:
        ...
        token_metric,token_metric_instance = load_predict(test_model,test_dataloader,token_model_save_dir,train_logger,index2label,tag,args)
        entity_metric,entity_metric_instance = load_predict(test_model,test_dataloader,entity_model_save_dir,train_logger,index2label,tag,args)
