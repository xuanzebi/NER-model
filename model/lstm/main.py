...
from model.helper.get_data import get_cyber_data, pregress,pregress_mtl,pregress_bert_embedding,pregress_mtl_mutltype
...
def train(model, train_dataloader, dev_dataloader, args, device, tb_writer, label_map, tag, train_logger,
    ...
    for epoch in tq:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.model_classes == 'bilstm':
                if args.use_bert or args.use_elmo:
                    input_ids, input_mask, label_ids,quid = _batch 
                    loss, _ = model(input_ids, input_mask, label_ids,quid,'train')
                    loss.backward()
                    optimizer.step()
                else:
...
def load_bert_embedding_data(data,word2idx,label2index,args):
    features = pregress_bert_embedding(data, word2idx, label2index,max_seq_lenth=args.max_seq_length)
    all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long)
    all_input_mask = torch.tensor([f.input_mask for f in features], dtype=torch.long)
    all_label_ids = torch.tensor([f.label_ids for f in features], dtype=torch.long)
    all_guids = torch.tensor([f.guid for f in features], dtype=torch.long)
    dataset = TensorDataset(all_input_ids, all_input_mask, all_label_ids,all_guids)
    return dataset
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_classes', type=str, default='bilstm_mtl',choices=['bilstm','bilstm_mtl','bilstm_start_end',"bilstm_data_mtl"], help='Which model to choose.')
    ...
    parser.add_argument('--adv_loss_type', default='', choices=['','fgm','vat','pgd','freelb','fgm_vat'], type=str)
    ...
    parser.add_argument('--FGM', default=True, type=str2bool, help='是否使用bert embedding')
    parser.add_argument('--use_token_mtl', default=True, type=str2bool, help='是否使用token级别多任务')
    parser.add_argument('--use_multi_token_mtl', default=False, type=str2bool, help='是否使用token级别多任务')
    parser.add_argument('--data_mtl', default=False, type=str2bool, help='是否使用数据级别多任务')
    parser.add_argument('--use_elmo', default=False, type=str2bool, help='是否使用elmo embedding')
    parser.add_argument('--use_bert', default=False, type=str2bool, help='是否使用bert embedding')
    ...
    if args.do_train:
        ...
        if args.model_classes == 'bilstm':
            if args.use_bert or args.use_elmo:
                train_dataset = load_bert_embedding_data(train_data_raw,word2idx,label2index,args)
                dev_dataset = load_bert_embedding_data(dev_data_raw,word2idx,label2index,args)
            ...
            if args.use_multi_token_mtl:
                train_data_id, train_mask_id, train_label_id,train_token_id = pregress_mtl_mutltype(train_data_raw, word2idx, label2index,max_seq_lenth=args.max_seq_length)
            else:
    ...
    if args.do_test:
        ...
        # Dataset
        if args.model_classes == 'bilstm':
            if args.use_elmo or args.use_bert:
                test_dataset = load_bert_embedding_data(test_data_raw,word2idx,label2index,args)
            ...
            if args.use_multi_token_mtl:
                test_data, test_mask, test_label,test_token_id = pregress_mtl_mutltype(test_data_raw, word2idx, label2index, max_seq_lenth=args.max_seq_length)
            else:
