...
class CNN_BASE(nn.Module):
    ...
    def forward(self, word_input, input_mask, labels,labels_token=None):
        ...
        if self.mode == 'GCNN':
            ...
            word_input = self.gated_resnet_torch(word_input,3,self.cnn_dim)
            word_input = word_input.transpose(2, 1).contiguous()
            cnn_output = self.inter_media(word_input)
        elif self.mode == 'CNN':
            cnn_output = self.cnn(word_input).transpose(2, 1).contiguous()
        elif self.mode == 'GLDR':
            word_input = self.cnn_glu(word_input)
            word_input = self.gated_linear_d_r(word_input,dilation=1,kernel_size=3,dim=word_input.shape[1])
            word_input = self.gated_linear_d_r(word_input,dilation=2,kernel_size=3,dim=word_input.shape[1])
            word_input = self.gated_linear_d_r(word_input,dilation=4,kernel_size=3,dim=word_input.shape[1])
            word_input = self.gated_linear_d_r(word_input,dilation=8,kernel_size=3,dim=word_input.shape[1])
            word_input = self.gated_linear_d_r(word_input,dilation=16,kernel_size=3,dim=word_input.shape[1])
            word_input = self.gated_linear_d_r(word_input,dilation=1,kernel_size=3,dim=word_input.shape[1])
            word_input = self.gated_linear_d_r(word_input,dilation=1,kernel_size=3,dim=word_input.shape[1])
        ...
        if self.use_crf:
            ...
            ans_loss = total_loss / batch_size
    ...
    def gated_resnet_torch(self,x, kernel_size, dim):
        ...
        conv_x = conv(x)
        x = x + F.glu(conv_x, 1)
        batchnorm = nn.BatchNorm1d(self.cnn_dim).to('cuda')
...
class Bilstmcrf_cnn(nn.Module):
    ...
    def forward(self, word_input, input_mask, labels):
        ...
        if self.use_cnn_fea:
            cnn_features = self.cnn_feature.get_output(word_input,input_mask)
